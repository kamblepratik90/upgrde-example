...
// Genosis safe
const safeAddress = "0x0e0751EEe6A1451ddc719EdE1a807b0444bF0eF0";
...
const createTrans = async () => {
  ...
  const myContract = new ethers.Contract(
    "0xD807C959Cd16d720e030904F31268A69386a93CB", // proxy
    // "0xA64142C327D26eE539cF26800c9EF56A1b86503e", // implementation
    Abi,
    owner1
  ); // Write only

  const transactions = await myContract.populateTransaction.setGreeting(
    "Hellooooo1"
  );
...
async function main() {
  try {
    ...
    const bal00 = await signerFunctions(owner1);
    const bal10 = await signerFunctions(signer2);
    console.log("bal0: ", ethers.utils.parseUnits(bal00, 0).toString());
    console.log("bal1: ", ethers.utils.parseUnits(bal10, 0).toString());
    // await deploy();
    await getSafe();
    await createTrans();

    const bal01 = await signerFunctions(owner1);
    const bal11 = await signerFunctions(signer2);

    const diff0 = ethers.utils
      .parseUnits(bal00, 0)
      .sub(ethers.utils.parseUnits(bal01, 0));
    const diff1 = ethers.utils
      .parseUnits(bal10, 0)
      .sub(ethers.utils.parseUnits(bal11, 0));

    console.log("dif0: ", diff0.toString());
    console.log("dif1: ", diff1.toString());
